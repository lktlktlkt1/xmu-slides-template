...
\usetheme{xmu}        % Madrid 基础 + 学术配色 + 排版宏（见 xmu-theme/）
...
% --- 占位辅助（仅模板用；真实稿件可删除）------------------------------------
% 行内占位文字
\newcommand{\ph}[1]{\textcolor{xmugrey}{〈#1〉}}
% 图片占位框（替换为 \fitfigure{your_image} 即可）
\newcommand{\phfig}{%
  \fcolorbox{cardgrey}{bgsoft}{%
    \parbox[c][0.42\textheight][c]{0.96\linewidth}{\centering
      \color{xmugrey}（图片占位 \textbar\ \texttt{\textbackslash fitfigure\{...\}}）}}}

% --- 元信息（替换为你的题目 / 作者）------------------------------------------
% 标题页字段（新版结构，见 xmu-theme 说明）：
...
\begin{frame}{实验设计}
  ...
  \begin{exampleblock}{评测协议}
    ...
    {\footnotesize\color{xmugrey} 基线：\ph{baseline A · B · C}}
  \end{exampleblock}
\end{frame}
...
\begin{frame}[plain,c]
  ...
  \deckgap
  % 与标题页底部一致（汇报人 · 地点 · 时间）
  {\color{xmugrey}\footnotesize \titlemetaline}
\end{frame}
...
\begin{frame}[plain,c]
  \centering
  {\color{xmugrey}\large Appendix}\\[0.6em]
